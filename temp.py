""" statement -> exprStmt | printStmt | block | IF statement | Fun declaration;
block = "{" declaration "}"
"""
#TODO: Need to change grammar for function declaration and function call
from Expr import *
from token_ import *
from Stmt import *
...
class Parser:
    ...
    def fun_declaration(self):
        ...
        name = self.primary()  # will return Indent
        body_part = None
        if self.peek().type == Tokentype.OPENBRA:
            self.advance()  # consume (
            arguments = []
            ...
            if self.peek().type == Tokentype.OPENPARA:
                self.advance() #consume {
                body_part = Block(self.block())
                return FunDec(name,arguments, body_part)
            else:
                print("Error form parser: missing {")
                exit()
        else:
            print("error from parser: missing ( for function declaration")
            exit()
    ...
    """
    def call_call(self):
        expr = self.primary() # primary should be INDENT TYPE
        while True:
            if self.peek().type == Tokentype.OPENBRA:
                self.advance() # consume (
                expr = self.call_helper(expr) # will return arguments
                # careful python does not support reassignment of variable in local scope.
                # it will create a new object which can result to return null object
                # check on runtime for null objects
            else:
                break
            #this will only return primary expression
        return expr
    ...
    def call_helper(self, expr:Expr):
        ...
        paren = self.peek()
        if self.peek().type != Tokentype.CLOSEBRA:
            print("Error from parser: Expect ')' after argumetns")
            exit()
        else:
            self.advance() # consume )
